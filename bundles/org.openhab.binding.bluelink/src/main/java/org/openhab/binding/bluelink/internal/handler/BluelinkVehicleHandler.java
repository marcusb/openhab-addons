...
public class BluelinkVehicleHandler extends BaseThingHandler {
    ...
    private @Nullable VehicleInfo vehicleInfo;
    ...
    public void initialize() {
        ...
        if (vin == null || vin.isBlank() || config.refreshInterval < 1) {
            updateStatus(ThingStatus.OFFLINE, ThingStatusDetail.CONFIGURATION_ERROR, "VIN is required");
            return;
        }

        final Bridge bridge = getBridge();
        if (bridge == null) {
            updateStatus(ThingStatus.OFFLINE, ThingStatusDetail.BRIDGE_UNINITIALIZED, "Bridge not configured");
            return;
        }
        ...
        final VehicleInfo vehicle = accountHandler.getVehicleByVin(vin);
        if (vehicle == null) {
        ...
        updateProperty(PROPERTY_VIN, vehicle.vin());
        updateProperty(PROPERTY_MODEL, vehicle.modelCode());
        updateProperty(PROPERTY_REGISTRATION_ID, vehicle.registrationId());
        updateProperty(PROPERTY_GENERATION, vehicle.vehicleGeneration());
        updateProperty(PROPERTY_ENGINE_TYPE, vehicle.evStatus());
        createDynamicChannels();
        updateStatus(ThingStatus.UNKNOWN);

        refreshJob = scheduler.scheduleWithFixedDelay(() -> refreshVehicleStatus(false), 0, config.refreshInterval,
                TimeUnit.MINUTES);
    }
    ...
    public void handleCommand(final ChannelUID channelUID, final Command command) {
        if (command instanceof RefreshType) {
            // we could forceRefresh to avoid returning stale cached data,
            // but there is a low daily request limit for those
            refreshVehicleStatus(false);
            return;
        }

        final BluelinkApi api = getApi();
        final VehicleInfo vehicle = vehicleInfo;
        if (api == null || vehicle == null) {
            logger.debug("Cannot handle command - API or vehicle not available");
            return;
        }
        ...
        try {
            switch (groupId) {
                case GROUP_STATUS -> handleStatusCommand(channelId, command, api, vehicle);
                case GROUP_CHARGING -> handleChargingCommand(channelId, command, api, vehicle);
                case null, default -> {
                }
            }
    ...
    private void handleStatusCommand(final String channelId, final Command command, final BluelinkApi api,
            final VehicleInfo vehicle) throws BluelinkApiException {
        if (CHANNEL_LOCKED.equals(channelId) && command instanceof OnOffType) {
            if (command == OnOffType.ON) {
                api.lockVehicle(vehicle);
            } else {
                api.unlockVehicle(vehicle);
            }
            scheduler.schedule(() -> refreshVehicleStatus(true), 10, TimeUnit.SECONDS);
        }
    }

    private void handleChargingCommand(final String channelId, final Command command, final BluelinkApi api,
            final VehicleInfo vehicle) throws BluelinkApiException {
        switch (channelId) {
            case CHANNEL_EV_CHARGING:
                switch (command) {
                    case OnOffType.ON:
                        api.startCharging(vehicle);
                        break;
                    case OnOffType.OFF:
                        api.stopCharging(vehicle);
                        break;
                    default:
                        // ignore
                }
            ...
            case CHANNEL_CHARGE_LIMIT_DC:
                if (command instanceof DecimalType decimalCommand) {
                    api.setChargeLimitDC(vehicle, decimalCommand.intValue());
                }
                break;

            case CHANNEL_CHARGE_LIMIT_AC:
                if (command instanceof DecimalType decimalCommand) {
                    api.setChargeLimitAC(vehicle, decimalCommand.intValue());
                }
                break;
        }
    ...
    public boolean climateStart(final QuantityType<Temperature> temperature, final boolean heat, final boolean defrost)
        ...
        if (api == null || vehicle == null) {
            logger.debug("API or vehicle not available");
            return false;
        }

        return api.climateStart(vehicle, temperature, heat, defrost);
    }
    ...
    public boolean climateStop() throws BluelinkApiException {
        ...
        return api.climateStop(vehicle);
    }
    ...
    public void refreshVehicleStatus(boolean forceRefresh) {
        ...
        try {
            ...
            final VehicleStatus status = api.getVehicleStatus(vehicle, forceRefresh);
            if (status != null) {
    ...
    private void updateChannels(final VehicleStatus status) {
        ...
        final VehicleInfo info = vehicleInfo;
    ...
    @Nullable
    public BluelinkApi getApi() {
        final Bridge bridge = getBridge();
        if (bridge == null) {
            return null;
        }
        final BluelinkAccountHandler handler = (BluelinkAccountHandler) bridge.getHandler();
    ...
    private void createDynamicChannels() {
        ...
        if (isElectric) {
            ...
            newChannels = List.of(
                    buildChannel(GROUP_CHARGING, CHANNEL_EV_BATTERY_SOC, "Number:Dimensionless", "ev-soc"),
                    buildChannel(GROUP_CHARGING, CHANNEL_EV_CHARGING, "Switch", "ev-charging"),
                    buildChannel(GROUP_CHARGING, CHANNEL_EV_PLUGGED_IN, "Switch", "ev-plugged-in"),
                    buildChannel(GROUP_CHARGING, CHANNEL_CHARGE_LIMIT_DC, "Number:Dimensionless", "charge-limit"),
                    buildChannel(GROUP_CHARGING, CHANNEL_CHARGE_LIMIT_AC, "Number:Dimensionless", "charge-limit"),

                    buildChannel(GROUP_CHARGING, CHANNEL_TIME_TO_FULL_CURRENT, "Number:Time", "charge-time"),
                    buildChannel(GROUP_CHARGING, CHANNEL_TIME_TO_FULL_FAST, "Number:Time", "charge-time"),
                    buildChannel(GROUP_CHARGING, CHANNEL_TIME_TO_FULL_PORTABLE, "Number:Time", "charge-time"),
                    buildChannel(GROUP_CHARGING, CHANNEL_TIME_TO_FULL_STATION, "Number:Time", "charge-time"));
        } else {
            group = GROUP_FUEL;
            newChannels = List.of(buildChannel(GROUP_FUEL, CHANNEL_FUEL_LEVEL, "Number:Dimensionless", "fuel-level"),
                    buildChannel(GROUP_FUEL, CHANNEL_FUEL_LOW_WARNING, "Switch", "warning"));
        }
        final Set<ChannelUID> currentChannels = getThing().getChannelsOfGroup(group).stream().map(Channel::getUID)
                .collect(toUnmodifiableSet());
        final ThingBuilder thingBuilder = editThing();
        newChannels.stream().filter(c -> !currentChannels.contains(c.getUID())).forEach(thingBuilder::withChannel);
        updateThing(thingBuilder.build());
    }

    private Channel buildChannel(final String group, final String channelId, final String itemType,
